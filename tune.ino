...
void TUNE_Run(void)
{
	...
	if ((TUNE_state == TUNE_BREAK) || (TUNE_state == TUNE_READY))
		return;
	...
	case 20:
		...
		if (best_cap_step > 0)
		{
			fine_cap_diff_low = coarse_cap_value[best_cap_step]
					- coarse_cap_value[best_cap_step - 1];
		}
		else
		{
			fine_cap_diff_low = 0;
		...
		{
			fine_cap_diff_high = coarse_cap_value[best_cap_step + 1]
					- coarse_cap_value[best_cap_step];
		}
		else
		{
		...
		if (best_ind_step > 0)
		{
			fine_ind_diff_low = coarse_ind_value[best_ind_step]
					- coarse_ind_value[best_ind_step - 1];
		}
		else
		{
			fine_ind_diff_low = 0;
		...
		{
			fine_ind_diff_high = coarse_ind_value[best_ind_step + 1]
					- coarse_ind_value[best_ind_step];
		}
		else
		{
